...
import { getSeniWithSanggarID } from '../db/SeniController'
import { getSanggarWithDocID } from '../db/SanggarController'

const CARD_WIDTH = Dimensions.get("window").width;

const SeniDetailScreen = ({navigation,route}:any) => {
  const {id, name, location, price, detail, tipe, sanggarID, imageURL} = route.params
  ...
  const providerTipe = "Sanggar";

  const handleProvider = async () => {
    try {
      setLoading(true);
      const providerDoc = await getSanggarWithDocID(sanggarID);
  ...
  const handleProduct = async () => {
    try {
      ...
      const querySnapshot = await getSeniWithSanggarID(sanggarID);
  ...
  return (
    ...
    <View style={styles.ScreenContainer}>
      ...
      <View style={{paddingHorizontal:SPACING.space_15, paddingVertical:SPACING.space_10}}>
        ...
        <Text style={styles.TextHeader2}>{formatedPrice(price)} / Bulan</Text>
        ...
        <View style={styles.line}/>
        {
          ...
          :
          <>
          <TouchableOpacity activeOpacity={0.7} onPress={()=>{navigation.push('SanggarDetail',
          {id:sanggarID, 
      ...
      <View style={{flexDirection:'row', justifyContent:'center', alignContent:'center', marginVertical:SPACING.space_10}}>
        ...
        <AppButton buttonStyle={{width:'90%', marginHorizontal:SPACING.space_10}} title="Daftar" backgroundColor={COLORS.primaryRedHex} textColor={COLORS.primaryWhiteHex} onPress={()=>{}}/>
      </View>
    </View>
...
export default SeniDetailScreen
