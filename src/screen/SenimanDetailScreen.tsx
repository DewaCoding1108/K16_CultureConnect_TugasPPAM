...
const SenimanDetailScreen = ({navigation,route}:any) => {
  const {id, name, location, address, description, tipe, phone, imageURL} = route.params
  const [results, setResults] = useState<any>([]); 
  const [loading,setLoading] = useState(true);

  const productTipe = "Jasa Seni";

  const handleProduct = async () => {
    setLoading(true);
    try {
      const querySnapshot = await getJasaSeniWithSenimanID(id);
...
export default SenimanDetailScreen
