    ...
    <View style={styles.ScreenContainer}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:SPACING.space_20, flexGrow:1}}>
          {Chart.map((item,index) =>
              <PesananCard key={index+1} screen="chart" buttonPressHandler={()=>{handlerNavigate(item)}} name={item.data.name} location= {item.data.detail} price={item.data.price} type= {item.data.category} imagelink= {item.data.imageURL} handleSecButton={() => {handlerDelete(item.id)}}></PesananCard>
          )}
        {renderBottomHeight()}        
      </ScrollView>
      {renderButton()}
