...
const JasaSeniDetailScreen = ({navigation,route}:any) => {
  const {id, name, price, detail, tipe, senimanID, imageURL, show} = route.params
  ...
  const providerTipe = "Seniman";

  const handleProvider = async () => {
    try {
      setLoading(true);
      const providerDoc = await getSenimanWithDocID(senimanID);
  ...
  const handleProduct = async () => {
    try {
      ...
      const querySnapshot = await getJasaSeniWithSenimanID(senimanID);
    ...
    <View style={styles.ScreenContainer}>
      ...
      <View style={{paddingHorizontal:SPACING.space_15, paddingVertical:SPACING.space_10}}>
        ...
        <View style={styles.line}/>
        {
          ...
          :
          <>
          <TouchableOpacity activeOpacity={0.7} onPress={()=>{navigation.push('SenimanDetail',
          {id:senimanID, 
...
export default JasaSeniDetailScreen
